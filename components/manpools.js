const Pool = Vue.component('poolcomponent', {
  ...
  <v-layout column>
    <v-toolbar color="pink" dark><v-toolbar-title>Pool Management</v-toolbar-title>
      <v-spacer></v-spacer>
      <v-btn @click="autogen">AutoGen Pools</v-btn>   
    </v-toolbar>
    <v-container fluid grid-list-md>
      <v-form v-model="valid" ref="form">
        <v-layout row wrap>
          ...
          <v-flex xs3><v-text-field label="Round" v-model="editedItem.round"></v-text-field></v-flex> 
      ...
      <div>
        <v-data-table :headers="headers" :items="pools" :pagination.sync="pagination" :search="search">
          ...
          <template slot="footer">
            <td colspan="100%"><strong>Notes: Just fill the blanks to register a new Pool</strong></td>
          </template>
        </v-data-table>
      </div>            
    </v-container>
...
`,
  data () {
    return {
      filterList: [ { type: 'head2head' }, { type: 'group' } ],
      menu: false,
      search: '',
      result: '',
      error: '',
      ...
      editedItem: { id: 0, pool_name: '', pool_type: '', entry_cost: 0, entry_quorum:0, entrants:0
                   ,pool_prize:0, payout:'',organiser:'', round:'' ,remarks: '' },
      pools: [],
  ...
  methods: { 
    ...
    autogen: function () {
      console.log("99) autogen");
      var postdata = { "op": "autoGenPool" };
      this.$http.post('/php/apiPool.php', postdata,{ headers: { 'Content-Type': 'application/json' } })
        .then(response => {
        },    response => {  });
    },
    getAllData: function () {
      ...
      var postdata = { op: "getOrgPools", organiser: this.organiser }; 
