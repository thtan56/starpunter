const MenuPlan = Vue.component('menuplan', {
  template: `
  <div>
    <h4>Menu Plan</h4>
      ...
      <v-layout row wrap>
        <v-flex md6 v-for="(value, key) in days">
          <v-card class="ma-2 pa-2">
            <draggable :id="key" v-model="recipes" :options="{group:'recipes'}" 
                        @start="drag=true" @end="drag=false" @add="newRecipe">
              <v-card-title class="headline">{{ capLetter(key) }}</v-card-title>
            </draggable>
            <v-card v-for="recipe in value">
              <v-card-title class="headline">{{ recipe[0].title }}</v-card-title>
            </v-card>
          </v-card>
        </v-flex>
      </v-layout>
