// userGames is the defined component in vue-router path
// full layout with top & side nav bars
const betGames = Vue.component('BetGamesComponent', {
  props: { 
  ...
  <v-content>
    ...
    <v-container fluid grid-list-lg>
      <v-layout row wrap>
        <v-flex d-flex xs12 sm12><v-card><v-card-text><betOgame2 :ticketid="ticketid"></betOgame2></v-card-text></v-card></v-flex>
      </v-layout>
    </v-container>
  </v-content>
`,
...
Vue.component('betOgame2', {
  ...
  <v-container fluid grid-list-md>
    <v-layout column>
      <v-toolbar color="pink" dark><v-toolbar-title>Betting Ticket {{ticketid}}</v-toolbar-title><v-spacer></v-spacer>
                  ...
                  <v-layout row wrap>
                    <v-flex xs6>
                      <v-card>
                        ...
                        <v-card-title primary-title>
                          <div class="headline">{{props.item.home_team}}</div>
                          <v-card-actions><v-btn icon @click="betItem(props.item,1)">{{props.item.home_odd}}</v-btn></v-card-actions>
                    ...
                    <v-flex xs6>
                      <v-card>
                        ...
                        <v-divider></v-divider>
                        <v-card-title primary-title>
                          <div class="headline">{{props.item.away_team}}</div>
                          <v-card-actions><v-btn icon @click="betItem(props.item,2)">{{props.item.away_odd}}</v-btn></v-card-actions>
...
//      pools: [],
      ...
      orgs: [ {name: 'NBA'},{name:'NBL'}, {name:'NFL'},{name:'AFL'}],
      checkedOrganisers:[],
...
//      this.editedItem.teams    = this.game.teams;
    ...
    getAllData: function () {
      ...
      this.$http.post('/php/apiGame.php', JSON.stringify(postdata), {
          ...
          console.log(this.games);        // null if new game created but no pools yet
