...
const loginCustomer=Vue.component('custComponent', {
  ...
  <v-content>
    ...
    <v-tabs-items v-model="tabs">
      ...
      <v-tab-item>Games<v-icon>games</v-icon><v-card><v-card-text>
        <betComponent status='pending' />              <!-- components/compBasketball.js (open) -->
      </v-card-text></v-card></v-tab-item>
  ...
  created () {
    console.log('1) customerinfo.js-customer: created');
...
const loginManager=Vue.component('managerComponent', {
  template: `
  <v-content>
    <topmenu></topmenu>
    <v-container fluid grid-list-md>
    ...
    <v-toolbar color="cyan" dark tabs>
      <v-toolbar-side-icon><img src="images/tristars2.png" width="70" height="40"></v-toolbar-side-icon>
      <v-toolbar-title>Manager Info</v-toolbar-title>
      <v-spacer></v-spacer>
  ...
  created () {
    console.log('1) customerinfo.js-manager: created');
    let qry = 'database/json_users_email.php?email=' + this.$store.state.loginUser.email;   //this.$route.params.userId;
    console.log(qry);   
    axios.get(qry)
      .then(response => { 
